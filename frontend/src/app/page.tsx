              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <AirportAutocomplete
                    ...
                    placeholder="Search by city or type code (e.g. JFK)"
                  />
                </div>
...
function AirportAutocomplete({
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Allow direct code entry — update parent on every keystroke
    onSelect(text.toUpperCase());
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const res = await fetch(`${API_URL}/api/airports/search?city=${encodeURIComponent(text)}`);
  ...
  return (
    <div className="relative">
      ...
      />
      {isOpen && (
        <ul className="absolute z-10 mt-1 w-full bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg shadow-lg max-h-48 overflow-y-auto">
          {suggestions.map((airport) => (
            <li
              key={airport.code}
              onMouseDown={() => handleSelect(airport)}
              className="px-4 py-2 cursor-pointer hover:bg-sky-50 dark:hover:bg-sky-900/30"
            >
